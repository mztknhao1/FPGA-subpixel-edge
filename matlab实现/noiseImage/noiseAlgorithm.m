...
%% -------------------------可配置参数------------------------------%
var = 0.0001;                                      %高斯噪声参数
I_dir = 'car.bmp';                            %读入图像的访问地址
threshold = 100;                                    %sobel算子阈值设置

%% ---------------------读入理想图像，加入噪声-----------------------%
I_ideal = imread(I_dir);                            %理想图像
I_ideal = im2double(I_ideal);
I_noise = imnoise(I_ideal,'gaussian',0,var);        %加入高斯噪声的图像
K = [1,2,1;2,4,2;1,2,1]./16;                        %平滑模板
G1 = imfilter(I_noise,K,'same').*255;               %G为平滑后的图像
...
subplot(222)
imshow(I_noise)
title(strcat('加噪声以后的图像，noise = ',num2str(var)));
subplot(223)
imshow(G./255)
title('高斯平滑后的图像')
subplot(224)
