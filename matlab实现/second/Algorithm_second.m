...
for h = 1:number
    ...
    if(cx>2 && cy>2 && cx<319 && cy<511)
        ...
        tmpc = (SL +SR - 2*SM)/2/(A-B);
        tmpa = (2*SM - 5*(A+B))/2/(A-B) - 1/12*tmpc;
        tmpb = (SR - SL)/2/(A-B);
        matlab_sim = [matlab_sim;cx,cy,tmpa,tmpb,tmpc,state];
    end
end

%% 通过计算每个边缘点离圆中心的半径与设定的理想半径之差来测试算法正确性
% [len,~] = size(matlab_sim);
% for i = 1:len
%     tmpstate = matlab_sim(i,6);
%     tmpcx = matlab_sim(i,1);
%     tmpcy = matlab_sim(i,2);
%     switch tmpstate
%         case  2                        %(0,45)
%             
%         case  1                        %(135,180)
...
K = 2 .* c ./ mu;
Ksim = 1/sqrt(60);
Kerr = sum((abs(K) - Ksim).^2).^0.5/length(b);
